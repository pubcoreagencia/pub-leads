...
async function markSavedLeads(userId: string, results: ExternalLead[]) {
  if (results.length === 0) {
    return [];
  }
    ...
    results.map(async (lead) => ({
      ...
      saved: Boolean(
        await findDuplicateLead(userId, {
          ...
          phone_2: lead.phone2,
          source: lead.source,
          source_place_id: lead.externalId,
        }),
      ),
    })),
  );
...
export async function POST(request: Request) {
  ...
  const params = parsed.data as LeadSourceSearchParams;
  ...
  try {
    ...
    const results = await overpassProvider.search(params);
    ...
    const message = getErrorMessage(error, "Erro ao buscar leads.");
