...
const schema = z.object({ city: z.string().trim().min(2), state: z.string().trim().min(2), niche: z.string().trim().min(2), limit: z.coerce.number().int().min(1).max(50).optional() });
