...
-- This file exists for historical compatibility and should not be treated as the current recommended schema.
...
create table if not exists public.plans (
  id text primary key,
  name text not null,
  type text not null check (type in ('free', 'mensal', 'anual', 'vitalicio')),
  price_cents integer not null default 0 check (price_cents >= 0),
  currency text not null default 'BRL',
  billing_interval text not null check (billing_interval in ('none', 'month', 'year', 'lifetime')),
  lead_limit integer,
...
create table if not exists public.leads (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  source text not null default 'openstreetmap' check (
...
create table if not exists public.lead_messages (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  lead_id uuid not null references public.leads(id) on delete cascade,
  channel text not null default 'whatsapp_manual' check (
...
check (status in ('new', 'qualified', 'contacted', 'responded', 'proposal', 'won', 'lost'));
...
check (pipeline_stage in ('new', 'qualified', 'contacted', 'responded', 'proposal', 'won', 'lost'));
...
values
  (
    ...
    1,
    1,
    '["100 leads", "5 buscas", "1 instancia WhatsApp", "1 pipeline"]'::jsonb,
    0
  ),
  (
    'mensal',
